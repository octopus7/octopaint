...
### Splash screen UI overlay

- 스플래시 배경과 투명 전경 캐릭터는 [`GENERATED_IMAGE_ASSETS.md`](GENERATED_IMAGE_ASSETS.md)에 기록된 별도 이미지 에셋을 각각 독립된 시각 레이어로 렌더링한다. 이미지 자체에는 텍스트나 버튼을 굽지 않는다.
- 전경 캐릭터는 어린 소녀 메이드이며, 상반신 중심으로 양손이 프레임 하단의 치마/앞치마 양옆을 잡고 정중히 인사하는 구도를 유지한다. 비성적이고 단정한 긴소매·높은 칼라 의상을 입은 정교한 고품질 애니메이션 일러스트 스타일로 표현하며, 은발 양갈래의 결·광택·그라데이션을 고품질로 채색한다. 하의 치마는 상의의 검은 부분과 같은 검은 천 재질로 하고 그 위에 흰 앞치마를 둔다. 앞치마 허리나 중앙에는 검은 리본을 달지 않으며, 흰 앞치마의 왼쪽 아래 끝과 오른쪽 아래 끝에 작은 검은 리본을 하나씩 총 2개 단다.
- 배경과 캐릭터 레이어에는 서로 다른 느린 translate/scale 애니메이션을 적용해 미세한 패럴랙스와 카메라 드리프트를 만든다. 움직임은 멀미를 유발하지 않도록 작은 변위와 배율 범위로 제한한다.
- 시스템 애니메이션이 비활성화되었거나 Reduce Motion 설정이 켜진 경우 애니메이션을 실행하지 않고 카탈로그의 1600×900 정적 폴백 합성 이미지로 전환한다.
- 프런트엔드 UI 레이어는 제작자 표기를 정확히 `by octopus7`로 렌더링한다. 이 표기는 애니메이션하지 않는다.
- 제작자 표기와 별도로 저장소 링크 버튼을 제공하며 대상 URL은 `https://github.com/octopus7/octopaint`다. 버튼은 애니메이션하지 않는 UI 오버레이다.
- 저장소 링크 버튼은 키보드로 포커스하고 실행할 수 있어야 하며, 실행하면 운영체제의 기본 브라우저에서 대상 URL을 연다.
...
- 앱 및 툴바 아이콘은 임의로 재생성하거나 대체하지 않는다. 구현자는 [`GENERATED_IMAGE_ASSETS.md`](GENERATED_IMAGE_ASSETS.md)에서 각 이미지의 목적과 최종 경로를 확인한 뒤 카탈로그에 기록된 파일을 꺼내 사용한다.
