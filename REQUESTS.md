...
# 요청 시작: 2026-08-06 18:25:21 KST | 작업 완료: 2026-08-06 18:35:50 KST (소요 시간: 0시간 10분 29초)

## 구현 내역

- Rectangular Marquee와 Elliptical Marquee는 드래그 후 놓을 때 선택 명령 한 번을 적용하도록 연결했다.
- Freehand Lasso는 포인터 중간 표본을 수집해 놓을 때 한 번 적용하고, Polygonal Lasso는 클릭 정점을 누적하도록 구현했다.
- Polygonal Lasso는 시작점 재클릭, 더블 클릭 또는 Enter로 확정하고 Escape로 진행 중 제스처를 취소하도록 구현했다.
- 활성 문서의 선택 경계 snapshot을 D3D 선분으로 투영하고 100ms 타이머로 Marching Ants phase를 전진시켰다.
- 도구·문서 전환과 포인터 취소·캡처 상실 시 진행 중 선택을 안전하게 정리하고, 애니메이션 렌더 실패가 UI 스레드를 종료시키지 않도록 방어했다.
- 전체 Debug x64 솔루션 빌드와 Core, Application, Domain, Tools, Layer, EditorState, Paint, Selection 테스트를 통과시켰다.
- 최신 실행 파일을 직접 실행해 3초 후에도 종료되지 않고 응답 상태임을 확인했다.

## 결정 내용

- 도구 버튼의 활성 표기와 실제 문서 선택 상태를 분리하고, 확정된 선택 영역은 움직이는 흑백 점선으로 표시한다.
